...
namespace Anthill.Inject
{
    ...
    public class AntInjectContainer : IInjectContainer, IServiceLocator, IInjector
    {
        private class TypeData
        {
            ...
            public List<KeyValuePair<InjectAttribute, PropertyInfo>> Properties { get; }
            public List<KeyValuePair<InjectAttribute, FieldInfo>> Fields { get; }
            ...
            public static TypeData Create(Type aType, bool aIsSingleton = false, object aInstance = null)
            {
                ...
                foreach (var field in aType.GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
                {
                ...
                foreach (var property in aType.GetProperties(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
                {
        ...
        public void RegisterSingleton(object aInstance, Type type, string aKey = null)
        {
            var typeData = TypeData.Create(type, true, aInstance);
        ...
        public void UnRegister(Type aType, Type aInterfaceType = null)
        {
            if (types.ContainsKey(aInterfaceType ?? aType))
            {
                types.Remove(aInterfaceType ?? aType);
            }

            if (typeDatas.ContainsKey(aType))
            {
                typeDatas.Remove(aType);
            }
        }
        ...
        #endregion InjectContainer Members
