namespace Anthill.Fsm
{
	...
	public class AntFsmAgent : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				ADebug.Warning($"AntFsmAgent `{gameObject.name}` have no FSM preset.");
				return;
			}
		...
		public void AddState(AntFsmState aState, bool aIsDefault)
		{
			...
			{
				ADebug.Warning($"Can't to add state `{aState.name}` because it is already exists!");
			}
		}
		...
		public void AddTransition(int aFromStateIndex, int aToStateIndex)
		{
			...
			{
				ADebug.Warning($"Transition from `{_states[aFromStateIndex].name}` to `{_states[aToStateIndex].name} already exists!");
			}
		...
		public bool PerformState(int aToStateIndex, bool aForce = false)
		{
			...
			{
				ADebug.Warning($"Can't find transition from `{_states[_currentStateIndex].name}` to `{_states[aToStateIndex].name}` state!");
			}
